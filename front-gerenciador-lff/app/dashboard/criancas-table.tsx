...
import { getCriancas } from '@/lib/db';

export function CriancasTable({ search, selectTab }: { search: string, selectTab: string }) {
  ...
  async function fetchCriancas(search: string = '', currentOffset: number = 0) {
    try {
      const { criancas, newOffset, totalCriancas } = await getCriancas(search, currentOffset, selectTab);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Faça a manutenção das crianças ativas e invativas no projeto.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
