...
import { CriancasEscolaTable } from '../criancas-escola-table';
...
export default function CustomersPage() {
    ...
    useEffect(() => {
        ...
        let id = queryParams.get("idEscola") ?? '';
        setIdEscola(id);
        if (id) {
            setLoadPage(true);
        }
        ...
        <Tabs defaultValue="active" value={selectedTab} onValueChange={handleTabChange}> {/* Controlando a aba selecionada */}
            ...
            <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex flex-col mt-3">
                    <Select value={idTurma} onValueChange={(value) => { setIdTurma(value) }}>
                        <SelectTrigger className="w-60">
                            <SelectValue placeholder="Selecione uma turma" />
                        </SelectTrigger>
                        <SelectContent>
                            {/* Adicionando uma opção vazia */}
                            <SelectItem value='0'>Selecione uma turma</SelectItem>
            ...
            <TabsContent value={selectedTab}>
                {loadPage ? (
                    <CriancasEscolaTable
                        ...
                        idEscola={idEscola}
                        nomeEscola={escolas.find(escola => escola.id == idEscola)?.nome.toUpperCase() || ''}
                        idTurma={idTurma}
                    />
                ) : null}
